...
export async function getSchedule({ forceRefresh = false } = {}) {
  ...
  scheduleLoadInFlight = (async () => {
    if (WEBAPP_URL) {
      try {
        // v2.1 getSchedule intentionally omits 30-minute slots that have already
        // ENDED, because they can no longer be booked or cancelled. The
        // reservation search still has to show them, so the unpruned history is
        // read in parallel from the SAME Apps Script using its existing,
        // read-only `getAll` action (no Apps Script change, no new route). It is
        // best-effort: a failure only means the history section stays empty.
        const [json, historyJson] = await Promise.all([
          fetchFromWebApp("getSchedule"),
          fetchFromWebApp("getAll").catch((e) => {
            console.warn("WebApp reservation history read failed", e.message)
            return null
          }),
        ])
        ...
        if (payload.reservations) {
          ...
          const historyPayload = historyJson?.data || historyJson
          const reservationHistory = historyPayload?.reservations
            ? normalizeReservations(historyPayload.reservations)
            : null
          ...
          // Any date the history knows about must stay selectable too.
          if (reservationHistory) {
            const historyDates = new Set(days)
            for (const key of Object.keys(reservationHistory)) {
              const [, date] = String(key).split('|')
              if (date) historyDates.add(date)
            }
            days = [...historyDates]
          }
...
// bypasses only the close-timing warning; `practiceLocations` tells the v2.1
// backend which locations count toward the practice-session limit.
export async function bookGroup({ location, date, courtId, slots, names, staffApproved = false, practiceLocations = null }) {
  const payload = { action: "bookGroup", location, date, courtId: String(courtId), slots, names, staffApproved: Boolean(staffApproved), practiceLocations }
  if (WEBAPP_URL) {
...
export async function toggleReservation({ location, date, courtId, slot, name, staffApproved = false, practiceLocations = null }) {
  if (WEBAPP_URL) {
    ...
    const payload = { action: "toggleReservation", location, date, courtId: String(courtId), slot, name, staffApproved: Boolean(staffApproved), practiceLocations }
    try {
      await fetchFromWebApp(null, payload)
    } catch (error) {
