...
async function fetchFromWebApp(action, postData) {
  ...
  const url = postData ? WEBAPP_URL : `${WEBAPP_URL}${separator}action=${action}`
  const opts = postData
    ? { method: "POST", headers: { "Content-Type": "text/plain;charset=utf-8" }, body: JSON.stringify(postData) }
    : {}
...
async function readReservationsUncached() {
  ...
  // Public fetch (no auth, read-only — bookings still cannot reach the sheet)
  const pub = await fetchPublicSheet()
  if (pub && pub.reservations) return { reservations: normalizeReservations(pub.reservations), source: "public" }
