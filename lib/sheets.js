...
// of the booking, or the Apps Script writes nothing at all. `staffApproved` and
// `activeLocations` are forwarded so the Apps Script can recheck the session
// rules against fresh sheet data while holding its write lock.
export async function bookGroup({ location, date, courtId, slots, names, staffApproved = false, activeLocations = [] }) {
  const payload = { action: "bookGroup", location, date, courtId: String(courtId), slots, names, staffApproved: staffApproved === true, activeLocations }
  if (WEBAPP_URL) {
...
export async function cancelGroup({ location, date, courtId, slots, names }) {
  const payload = { action: "cancelGroup", location, date, courtId: String(courtId), slots, names }
  if (WEBAPP_URL) {
    await fetchFromWebApp(null, payload)
    invalidateScheduleCache()
    return null
