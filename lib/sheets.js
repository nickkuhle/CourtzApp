...
// cell discovery as the parser. Supports shared occupancy up to 4 players per
// 30-minute slot (previously it rejected any partial occupancy).
// `cancel` clears matching names instead of writing them. This path is not
// transactional per-cell like Apps Script's LockService version, so it
// re-checks empty capacity before writing.
async function writeGroupToGrid(sheets, { location, date, courtId, slots, names, cancel = false }) {
  ...
  if (!cleanNames.length) throw new Error('No players given')
  const cellsPerSlot = ranges.length / slots.length
  if (!Number.isFinite(cellsPerSlot) || cellsPerSlot <= 0) {
    throw new Error(`Unable to determine court capacity for Court ${courtId}`)
  }
  ...
  // Group cells by slot and validate occupancy per slot (allows 4 players total)
  const writes = []
  for (let si = 0; si < slots.length; si++) {
    const slotLabel = slots[si]
    const slotCells = ranges.slice(si * cellsPerSlot, (si + 1) * cellsPerSlot)
    const occupied = slotCells.filter(c => c.value !== "")
    // Prevent duplicate booking of same player in same slot
    for (const name of cleanNames) {
      if (occupied.some(c => c.value === name)) {
        throw new Error(`Slot ${slotLabel} is already booked by ${name} on Court ${courtId}`)
      }
    }
    const emptyCells = slotCells.filter(c => c.value === "")
    if (emptyCells.length < cleanNames.length) {
      throw new Error(`Slot ${slotLabel} on Court ${courtId} only has ${emptyCells.length} open spot(s) for ${cleanNames.length} player(s) (already ${occupied.length}/${slotCells.length} booked)`)
    }
    // Assign each new player to an empty cell in this slot
    for (let ni = 0; ni < cleanNames.length; ni++) {
      writes.push({ a1: emptyCells[ni].a1, name: cleanNames[ni] })
    }
  }
