...
export async function getSchedule({ forceRefresh = false } = {}) {
  ...
  scheduleLoadInFlight = (async () => {
    if (WEBAPP_URL) {
      try {
        // getSchedule is the bookable payload: v2.1 deliberately prunes every
        // 30-minute slot that has already ENDED, because those can no longer be
        // booked or cancelled. getAll is the unpruned read-only mirror of the
        // same grid, so it is fetched alongside purely so the reservation
        // SEARCH can still show past sessions. Both are existing GET actions —
        ...
        const [json, historyJson] = await Promise.all([
          ...
          fetchFromWebApp("getAll").catch(e => {
            console.warn("WebApp history read failed", e.message)
            return null
          }),
        ])
        ...
        if (payload.reservations) {
          ...
          const history = historyJson?.reservations
            ? normalizeReservations(historyJson.reservations)
            : reservations
          ...
          return cacheSchedule({
            reservations,
            // Read-only superset used by the reservation search only. Bookings
            // and cancellations always work off `reservations`.
            history,
            roster,
