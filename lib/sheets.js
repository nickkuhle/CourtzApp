...
export async function readReservations() {
  let webAppReservations = null
  if (WEBAPP_URL) {
    try {
      const json = await fetchFromWebApp("getAll")
      if (json.reservations && Object.keys(json.reservations).length) webAppReservations = normalizeReservations(json.reservations)
    } catch (e) { console.warn("WebApp read failed", e.message) }
  }
  // Always also try public fallback to ensure all dates (fixes 2001 bug even if WebApp is stale)
  let publicReservations = null
  try {
    const pub = await fetchPublicSheet()
    if (pub && pub.reservations && Object.keys(pub.reservations).length) publicReservations = normalizeReservations(pub.reservations)
  } catch {}
  // Merge: public has correct 2001->2026 fix, so merge both
  if (webAppReservations && publicReservations) return mergeReservations(webAppReservations, publicReservations)
  if (webAppReservations) return webAppReservations
  if (publicReservations) return publicReservations
...
export async function toggleReservation({ location, date, courtId, slot, name }) {
  if (WEBAPP_URL) {
    const json = await fetchFromWebApp(null, { action: "toggleReservation", location, date, courtId: String(courtId), slot, name })
    if (json.reservations) return normalizeReservations(json.reservations)
  ...
  return localToggle({ location, date, courtId, slot, name })
}
