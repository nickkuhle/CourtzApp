...
// Resolves the authoritative action for a TAP ON A TIME SLOT in a court card.
// The parent page calls this with its synchronously updated selected-player
// value, rather than trusting the mode/players captured by a court-card render
// that can be one React update behind a just-completed "Booking Courts As"
// switch. In the reported flow (Reena books 5:30-6:00, the card is reopened,
// the switcher moves to Jordyn, then the same slot is tapped) this turns the
// stale `cancel for Reena` payload into a `book for Jordyn` intent, because
// occupancy belongs to the slot while ownership belongs to the currently
// selected player. Explicit per-player cancel (x) buttons do not use this
// helper and therefore keep their targeted cancellation behavior.
export function getSlotTapIntent(reservedBy, currentPlayer) {
  const players = [...new Set(
    (Array.isArray(reservedBy) ? reservedBy : [])
  ...
  return {
    mode: state.action,
    // Cancelling a slot the selected player owns presents the whole group in
    // the dialog (players can be removed individually there). Booking a slot
    // presents only the selected player, or nobody when the switcher is blank.
