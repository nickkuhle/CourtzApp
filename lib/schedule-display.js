...
export const PLAYER_STYLES = [
  { avatar: 'bg-sky-600 text-white', chip: 'border-sky-200 bg-sky-50 text-sky-800', dot: 'bg-sky-500', fill: '#0284c7' },
  { avatar: 'bg-violet-600 text-white', chip: 'border-violet-200 bg-violet-50 text-violet-800', dot: 'bg-violet-500', fill: '#7c3aed' },
  { avatar: 'bg-rose-600 text-white', chip: 'border-rose-200 bg-rose-50 text-rose-800', dot: 'bg-rose-500', fill: '#e11d48' },
  { avatar: 'bg-amber-600 text-white', chip: 'border-amber-200 bg-amber-50 text-amber-900', dot: 'bg-amber-500', fill: '#d97706' },
  { avatar: 'bg-teal-600 text-white', chip: 'border-teal-200 bg-teal-50 text-teal-800', dot: 'bg-teal-500', fill: '#0d9488' },
  { avatar: 'bg-indigo-600 text-white', chip: 'border-indigo-200 bg-indigo-50 text-indigo-800', dot: 'bg-indigo-500', fill: '#4f46e5' },
  { avatar: 'bg-pink-600 text-white', chip: 'border-pink-200 bg-pink-50 text-pink-800', dot: 'bg-pink-500', fill: '#db2777' },
  { avatar: 'bg-lime-700 text-white', chip: 'border-lime-200 bg-lime-50 text-lime-900', dot: 'bg-lime-600', fill: '#4d7c0f' },
]
...
// Turns one court's grouped sessions into the display blocks the UI renders.
// Players sharing an identical block are merged into one `players` array.
function blocksFromSessions(sessions, { dateKey, location, court }) {
...
export function courtSessionBlocks(reservations, { dateKey, location, court }) {
  ...
  return blocksFromSessions(sessions, { dateKey, location, court })
}

// Computes the display blocks for EVERY court at one location + day in a single
// pass. `courtSessionBlocks` re-scans the whole reservations object for each
// court, so a 12-court grid paid the full session-grouping cost 12 times; this
// groups the location once and hands back a Map(courtId -> blocks) instead.
export function courtSessionBlocksByCourt(reservations, { dateKey, location }) {
  const result = new Map()
  if (!dateKey || !location) return result
  ...
  for (const session of sessions) {
    const court = String(session.court)
    if (!byCourt.has(court)) byCourt.set(court, [])
    byCourt.get(court).push(session)
  }

  for (const [court, courtSessions] of byCourt) {
    result.set(court, blocksFromSessions(courtSessions, { dateKey, location, court }))
  }
  return result
}

export function formatPlayerFirstName(value) {
  const full = formatPlayerName(value)
  const first = full.split(/\s+/).filter(Boolean)[0]
  return first || full
}

// Just the surname for the compact on-court overlay. Handles both canonical
// roster forms: "Last, First" -> "Last", "First Last" -> "Last" (the final
// word), and a single-word name stays as-is.
export function formatPlayerLastName(value) {
...
// Picks the reserved session a court card should land on: the session covering
// "now" when one exists, otherwise the next upcoming session. Past-only days
// fall back to the first block so the carousel still has a starting card.
export function describeFocusedSession(blocks, { dateKey, nowMs = Date.now() } = {}) {
  const list = Array.isArray(blocks) ? blocks : []
  if (!list.length || !dateKey) return { index: -1, kind: null, block: null }

  const now = laNow(nowMs)
  if (dateKey === now.dateKey) {
    const current = list.findIndex((block) => isCurrentBlock(block, now.minutes))
    if (current >= 0) return { index: current, kind: 'current', block: list[current] }
    const next = list.findIndex((block) => Number.isFinite(Number(block.start)) && Number(block.start) > now.minutes)
...
// Up to four players from the reservation that is on court right now.
export function currentReservationPlayers(blocks, { dateKey, nowMs = Date.now() } = {}) {
  const focused = describeFocusedSession(blocks, { dateKey, nowMs })
  if (focused.kind !== 'current' || !focused.block) return []
  return (focused.block.players || []).filter(Boolean).slice(0, 4)
}
